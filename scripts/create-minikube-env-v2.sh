...
HAVE_PROFILE=""
for profile in $(minikube profile list -l -o json | jq -r '.valid[].Name')
do
    if [ $profile == $EVT ]; then
        HAVE_PROFILE="true"
    fi
...
if [ ! -z $HAVE_PROFILE ]  && [ ! -z $DELETE_PROFILE ]; then
...
STATUS=$(minikube profile list  -o json | \
    jq -r ".valid[] | select(.Name == \"$EVT\").Status")
if [ $STATUS != "Running" ]; then
    minikube start --profile="${EVT}" --wait=all
fi
...
# ---------------------------------------------------------------------------- #
CONFIGURED=$(kubectl -n metallb-system get cm config -o json | \
    jq -r .data.config | grep ${MINIKUBE_IP})
...
[ ! -d "${CHARTS_REPO_BASE}/nfs-subdir-external-provisioner" ] && \
    ( echo "missing nfs-subdir-external-provisioner chart" && exit 2 )

INSTALLED=$(kubectl -n nfs-provisioning get pods \
    -l "app=nfs-subdir-external-provisioner" -o name | wc -l)
...
if [ $? -ne 0 ]; then
    KEYNAME=$(vault kv get -format json kv/${EVT}/vault/akv-unseal-key | \
